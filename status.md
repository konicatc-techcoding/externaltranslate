...
### 手動字幕（2026-08-12，使用者提出，已實作＋實測）

需求原文：送出後停留、可以存下來、5 個輸出欄位、一個發送 ON AIR 鍵；
選第 2 格改完內容按發送，畫面就換成第 2 格。**預存語句庫先不做**（見下方延後項目）。

**架構上幾乎沒有阻力，因為 `VmixOutput` 本來就是自足的**——自己的 client、自己的節流
送出器、自己的狀態機。開第二個實例指向另一個 `input_guid`，那個 class 一行都沒改。

真正要改的是**生命週期**：vMix 輸出原本綁在翻譯上（`_open_vmix_output()` 在 `start()`
裡、`_close_vmix_output()` 在結束的 `finally` 裡），沒在翻譯時根本不存在輸出。手動字幕
有自己的 `_manual_output`，與 pipeline 無關，**停止翻譯時照樣能送**。

- **設定**：`vmix.manual_input_guid`／`manual_input_name`／`manual_fields`／`manual_slots`
  （5 格，各 200 字內）。host/port 共用——是同一台 vMix。
- **擋住「手動與翻譯指到同一個 input」**：兩邊會互相蓋，畫面上看起來像隨機閃爍。
  config 層 fail closed，面板的下拉選單也直接把翻譯用的那個濾掉。
- **輸出目標的選單在手動字幕卡裡**（2026-08-12 使用者要求搬過來，原本在 vMix 輸出面板）。
  搬過來就必須讓**面板無條件顯示**——原本是「選好 Title 才出現」，選單搬進去就永遠選不到。
  順帶修掉一個更根本的問題：找不到功能的人得不到任何說明。現在沒選 Title 時面板照樣在，
  只是送出鍵停用，並說明缺什麼（沒讀到 input／還沒選 Title）。
  讀取 input 的按鈕也複製一份到這張卡，免得為了一個動作跳去另一張卡。
- **每行字數獨立**（`vmix.manual_chars_per_line`，4–60，預設 20）。原先沿用翻譯的
  `caption.chars_per_line`，那是耦合缺陷：調翻譯的字數會讓手動字幕跟著重排，而兩者是
  不同的 GT Title、各有文字框與字級。**刻意不做「跟隨翻譯」模式**——多一個模式就多一個
  「它為什麼自己變了」的問題。
  斷行預覽刻意不做即時版：斷行演算法只在後端（全形寬、中文標點禁則），前端重寫一份就
  破壞了「所有輸出共用同一份斷行」這條 Stage 3.1 的原則。改為送出後在面板列出實際的
  每一行，不合適就改數字再送一次。
- **排版保留開頭，不是結尾**。翻譯視窗是滑動的，最新的字才該留；打好的訊息是固定文字，
  砍掉開頭等於藏掉重點。超出欄位數時送前面幾行並在面板明講。
  句尾換行對手動字幕關閉——那條規則是為串流中的句子設計的，用在固定訊息上只是浪費格子。
- **送出即存檔**：送出是「這個寫法就是要的」最清楚的訊號，不必另外按儲存。
  失焦也會存。存進 `user.yaml`，重開程式還在。
- **`close_manual_output()` 刻意不清空畫面**。翻譯輸出在停止時清空是因為凍住的即時字幕
  會誤導；手動訊息是人特意放上去的，要不要撤是操作者的事。

**不同電腦的情況**：在那台跑第二份程式即可——GT Title 跨機器已實證，那台不開始翻譯、
不需要 API Key 與音訊裝置，`vmix.host` 指同一台 vMix、`manual_input_guid` 指另一個 Title。
**不會為此把控制頁攤上區網**：控制頁能開關翻譯、能設定 API Key，比 overlay 的情況更嚴重。

實測（2026-08-12，假 vMix ＋ 真實瀏覽器面板）：點選第 2 格 → 按發送 ON AIR →
假伺服器收到 `bbbb-2222 / Manual1.Text = "節目稍後開始"`、`Manual2.Text = ""`，
**翻譯用的 input 全程沒有任何寫入**。

**延後項目（使用者 2026-08-12 說先記下來）**：預存語句庫——一份可命名、可增刪的常用字幕
清單，點一下直接送出。目前的 5 格已能涵蓋大部分需求，語句庫是它的放大版。
**未經指示不得動工。**
...
>   使用者尚未指示開始，勿自行動工。
> - **手動跑馬（2026-08-12 使用者決定先不做）**：關鍵結論是 **`SetText` 做不出跑馬**
>   ——那條管道是整段換字，自己推字元只會得到字元級跳動加上每秒十幾個請求。
>   只有 Browser Input（自己畫，限同機）或 vMix 內建跑馬 Title（跨機可用）兩條路。
>   完整評估與動工前必須先問的六個問題寫在 `PLAN.md`「候選功能：手動跑馬」。
>   **未經新的明確指示不得動工。**
