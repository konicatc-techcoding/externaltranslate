...
- **Stage 3.1（驗證中）**：後端字幕排版器——以全形寬計算的「每行字數 × 行數」、中文標點
  禁則、拉丁字詞不硬拆、滑動視窗，以及**句尾換行**（`。！？` 結束時該行剩餘不足 4 個全形字
  就讓下一句另起一行）。控制頁可於翻譯進行中即時調整並重新排版；overlay 與
...
- 自動建立或修改 vMix 的 input 與 GT Title 版面。
...
`。！？` 結束一句時，若該行**剩下不足 4 個全形字**，下一句從新的一行開始；剩下夠多就接著排。

沒有這條規則時，新句子會擠在行尾然後立刻折行：
...
**門檻用「剩餘空間」而不是「行的幾分之幾」**。以比例衡量在長行會失控：每行 60 字時
「超過 2/3 就換行」代表句子在第 41 字結束就丟掉將近 20 字的可用空間，而那行明明還放得下
大半句話。以剩餘 4 個全形字為準，不論每行 10 字或 60 字，留白都不超過 4 字。

**只認全形的 `。！？`**。半形句點出現在「3.5 公里」「Mr. Chen」裡，拿它當句尾會把數字剖半。
句尾後面緊跟的 `」』）` 算在同一句內，所以「他說「好的。」」不會在引號前斷開。

判斷**只看已經顯示出來的內容**，不看下一句有多長。字幕是一個片段一個片段到達的，下一句
通常還在打字中；用它的長度做判斷會讓已經定案的斷行隨著後續文字反覆變動，在讀者眼前跳來跳去。

也可用 API 直接改：

```bash
curl -X PUT http://127.0.0.1:8765/api/settings/caption-layout -H "Content-Type: application/json" -d "{\"chars_per_line\":10,\"max_lines\":2,\"sentence_breaks\":true}"
```
...
| 設定 | 說明 |
|---|---|
| 啟用 | 總開關，對應 `features.vmix_output` |
| 主機／連接埠 | 預設 `127.0.0.1:8088`；只收裸主機名或 IP，**不接受完整網址** |
| 輸出到哪個 input | 從 vMix 讀取清單後用名稱選，程式**記 GUID** |
| 文字欄位名稱 | 一行一個，對應字幕的每一行 |

### 手動字幕

左側的「手動字幕」面板：先按「從 vMix 讀取 input」，選一個**與翻譯不同**的 Title，
下面是 5 個可編輯的欄位和一個「發送 ON AIR」。選一格、需要的話改內容、按發送，
那句話就會出現在該 Title 上並**一直停留**，直到再送一次或按清空。

- 與即時翻譯**同時進行、互不影響**：兩者寫不同的 input，任一邊出問題都不會波及另一邊。
- 5 格內容會存進設定檔，重開程式還在；**送出時會一併存下**，不必另外按儲存。
- 手動 Title **不能和翻譯共用**——兩邊會互相蓋掉，設定層會直接拒絕。
- **每行字數是手動字幕自己的**，與即時翻譯各自獨立——兩者是不同的 Title，各有自己的
  文字框與字級。送出後面板會列出實際斷成的每一行，不合適就改數字再送一次。
- 文字超過該 Title 的欄位數時，送出前面幾行並在面板提示。

**操作者在另一台電腦時**：在那台跑第二份程式即可。那台不需要開始翻譯、不需要 API Key
與音訊裝置，只要 `vMix 主機` 指向同一台 vMix、手動字幕的 Title 選另一個就行。

### 本程式是欄位規格的來源

面板的「依目前行數產生欄位名稱」會產生 `Line1.Text … LineN.Text`，**你照著在 vMix 的
Title Editor 建立同名欄位**即可。GT Title 的文字欄位要加 `.Text` 後綴。名稱也可以改成
`Headline.Text` 這類既有命名——清單是權威，產生只是捷徑。

欄位比行數少時只會顯示最後幾行，面板會當場標示；欄位比行數多時，多出來的會被送空字串清掉，
否則上一句的殘留會留在畫面上。

### 記 GUID 不記編號

vMix 的 input 可以用編號、名稱或 GUID 指定。本程式**只用 GUID**：編號會在新增或刪除 input
時位移，之後 `SetText` 會安靜地寫到別的 input——畫面出現錯的字幕而且不會報錯。啟動輸出前會
先確認該 GUID 還在，不在就停用輸出並說明，不猜、不退回 input 1。

### 更新節流

字幕每秒更新數次。送出器只保留最新狀態，每 `min_interval_ms`（預設 200 ms）最多送一次，
相同內容跳過。**節流窗口結束時一定會補送最後一筆**——只丟棄過快的更新而不補尾筆，字幕會
永遠停在倒數第二個片段。

### 失敗一律隔離

vMix 沒開、變慢、回 500 或中途關掉，都**不影響翻譯、網頁 overlay 或本機控制服務**。
送出在獨立 task，失敗進 bounded backoff（`0.5 → 1 → 2 → 4 → 5` 秒上限），
狀態列的 `vmix_output` 會顯示 backoff，控制頁面板顯示原因。

### 停止時會清空 GT Title

停止翻譯、清空字幕、關閉輸出三條路徑都會送空字串。否則最後一句會**永遠留在 vMix 畫面上**，
看起來像翻譯還在跑。

### vMix 在另一台機器

支援，但**vMix Web API 只有 HTTP、沒有加密**——指定非本機位址時，翻譯後的字幕會以明文
經過區域網路。面板會標示這件事。本機控制服務本身仍然只綁 `127.0.0.1`：這裡放寬的是我們
主動連出去的目的地，不是別人能連進來的入口。

### 尚未驗證

假 vMix 是照官方文件寫的，它只能證明「我們送出的東西符合文件描述」。**GT Title 欄位名稱的
`.Text` 後綴、單欄位多行的顯示方式、中文在 GT Title 的字型表現，都要在真的 vMix 上確認**
（Phase B）。
